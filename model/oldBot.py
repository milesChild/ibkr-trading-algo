...
class Bot:
    ...
    def __init__(self):
        ...
        if (int(self.barsize) > 1):
            mintext = " mins"
        queryTime = (datetime.now().astimezone(pytz.timezone("America/New_York")) - timedelta(days=1)).replace(hour=16,
                                                                                                               minute=0,
                                                                                                               second=0,
    ...
    def on_bar_update(self, reqId, bar, realtime):
        ...
        if (realtime == False):
            ...
            if (minutes_diff > 0 and math.floor(minutes_diff) % self.barsize == 0):
                ...
                self.sma = ta.trend._sma(self.close_array, self.smaPeriod, True)
                ...
                # Testing
                print(self.currentBar.close, " > ", lastHigh)
                print(self.currentBar.low, " > ", lastLow)
                print(self.currentBar.close, " > ", self.sma[len(self.sma) - 1])
                print(lastClose, " < ", self.sma[len(self.sma) - 2])

                # Check Criteria
                if (self.currentBar.close > lastHigh
                        and self.currentBar.low > lastLow
                        and self.currentBar.close > self.sma[len(self.sma) - 1]
                        and lastClose < self.sma[len(self.sma) - 2]):
                    print("Strategy Triggered")
                    # Bracket Order 2% Profit Target 1% Stop Loss
                    profitTarget = bar.close * 1.005
                    stopLoss = bar.close * 0.9975
                    quantity = 10
